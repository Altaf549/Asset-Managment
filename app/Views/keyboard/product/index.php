...
<html>
<head>
    <title>Keyboard Product Management - Shopping Admin</title>
...
<body>
    <div class="container-fluid">
        <div class="row">
            <?= view('admin/templates/sidebar', ['page' => 'keyboard']) ?>
            <div class="col-md-9 col-lg-10 main-content">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2>Keyboard Product Management</h2>
                    <div class="d-flex gap-2">
                        <button class="btn btn-primary me-2" onclick="openCreateModal()">
                            <i class="fas fa-plus"></i> Add Keyboard
                        </button>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="keyboardTable" class="table table-striped table-hover" style="width: 100%;">
                            <thead>
                                <tr>
                                    <th>SL No.</th>
                                    <th>Asset ID</th>
    ...
    <?= view('keyboard/product/modal') ?>
    <?= view('keyboard/product/assign_modal') ?>
    <?= view('keyboard/product/unassign_modal') ?>
    ...
    <script>
        ...
        function initializeTable() {
            ...
            keyboardTable = $('#keyboardTable').DataTable({
                pageLength: 10,
                lengthChange: false,
                searching: true,
                ordering: true,
        ...
        function openAssignModal() {
            ...
            // Fetch asset data
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/getAllUnassignKeyboardProducts') ?>',
                method: 'GET',
                success: function(response) {
        ...
        function loadKeyboards() {
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/getAllKeyboardProducts') ?>',
                method: 'GET',
                success: function(response) {
                    keyboardTable.clear();
                    response.data.forEach((keyboard, index) => {
                        const statusToggle = `
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" 
                                    id="status_${keyboard.id}" 
                                    ${keyboard.assign_status === 'yes' ? 'checked' : ''}
                                    disabled>
                            </div>
                        `;
                        ...
                        const actions = `
                            <button class="btn btn-sm btn-warning me-2" 
                                onclick="editKeyboard(${keyboard.id})">
                        ...
                        keyboardTable.row.add([
                            index + 1,
                            keyboard.asset_id || '',
                            keyboard.manufacturer || '',
                            keyboard.keyboard_type || '',
                            keyboard.assigned_to || '',
                            keyboard.emp_id || '',
                            formatDate(keyboard.assign_date),
                            statusToggle,
                            formatDate(keyboard.created_at),
                            actions
                        ]);
                    });
                    keyboardTable.draw();
        ...
        function openCreateModal() {
            $('#keyboardId').val('');
            $('#keyboardForm')[0].reset();
            $('#keyboardModal').modal('show');
        }

        function editKeyboard(id) {
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/') ?>' + id,
                method: 'GET',
                success: function(response) {
                    $('#keyboardId').val(response.id);
                    $('#asset_id').val(response.asset_id);
                    $('#manufacturer').val(response.manufacturer);
                    $('#keyboard_type').val(response.keyboard_type);
        ...
        function saveKeyboard() {
            const id = $('#keyboardId').val();
            const url = id ? '<?= base_url('admin/keyboard/product/updateKeyboardProduct/') ?>' + id : '<?= base_url('admin/keyboard/product/createKeyboardProduct') ?>';
            ...
            $.ajax({
                ...
                data: {
                    ...
                    keyboard_type: $('#keyboard_type').val(),
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        loadKeyboards();
                        toastr.success('Keyboard ' + (id ? 'updated' : 'created') + ' successfully');
                    } else {
        ...
        function deleteKeyboard(id) {
            if (confirm('Are you sure you want to delete this keyboard?')) {
                $.ajax({
                    url: '<?= base_url('admin/keyboard/product/deleteKeyboardProduct/') ?>' + id,
                    method: 'DELETE',
                    success: function(response) {
                        if (response.success) {
                            loadKeyboards();
                            toastr.success('Keyboard deleted successfully');
                        } else {
                            toastr.error(response.message);
                        }
                    }
        ...
        function toggleStatus(id) {
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/updateKeyboardProduct/') ?>' + id,
                method: 'POST',
                data: {
        ...
        function assignKeyboard() {
            ...
            // Make AJAX call
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/assignKeyboard') ?>',
                method: 'POST',
                data: data,
                success: function(response) {
                    if (response.success) {
                        ...
                        loadKeyboards(); // Refresh the table
                    } else {
                        let errorMessage = response.message || 'Failed to assign keyboard';
                        if (response.errors) {
                ...
                error: function(xhr, status, error) {
                    let errorMessage = 'Error occurred while assigning keyboard';
                    try {
        ...
        function openUnassignModal() {
            ...
            // Fetch asset data
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/getAllAssignKeyboardProducts') ?>',
                method: 'GET',
                success: function(response) {
                    response.data.forEach(asset => {
                        assetSelect.append(
        ...
        // Function to handle unassign keyboard
        function unassignKeyboard() {
            ...
            // Make AJAX call
            $.ajax({
                url: '<?= base_url('admin/keyboard/product/unassignKeyboard') ?>',
                method: 'POST',
                data: data,
                success: function(response) {
                    if (response.success) {
