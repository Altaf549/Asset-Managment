...
<html>
<head>
    <title>Mouse Product Management - Shopping Admin</title>
...
<body>
    <div class="container-fluid">
        <div class="row">
            <?= view('admin/templates/sidebar', ['page' => 'mouse']) ?>
            <div class="col-md-9 col-lg-10 main-content">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2>Mouse Product Management</h2>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="mouseTable" class="table table-striped table-hover" style="width: 100%;">
                            <thead>
                                <tr>
                                    <th>SL No.</th>
                                    <th>Asset ID</th>
    ...
    <?= view('mouse/product/modal') ?>
    <?= view('mouse/product/assign_modal') ?>
    <?= view('mouse/product/unassign_modal') ?>
    ...
    <script>
        ...
        function initializeTable() {
            ...
            mouseTable = $('#mouseTable').DataTable({
                pageLength: 10,
                lengthChange: false,
                searching: true,
                ordering: true,
        ...
        function openAssignModal() {
            ...
            // Fetch asset data
            $.ajax({
                url: '<?= base_url('admin/mouse/product/getAllUnassignMouseProducts') ?>',
                method: 'GET',
                success: function(response) {
        ...
        function loadMouses() {
            $.ajax({
                url: '<?= base_url('admin/mouse/product/getAllMouseProducts') ?>',
                method: 'GET',
                success: function(response) {
                    mouseTable.clear();
                    response.data.forEach((mouse, index) => {
                        const statusToggle = `
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" 
                                    id="status_${mouse.id}" 
                                    ${mouse.assign_status === 'yes' ? 'checked' : ''}
                                    disabled>
                            </div>
                        `;
                        ...
                        const actions = `
                            <button class="btn btn-sm btn-warning me-2" 
                                onclick="editMouse(${mouse.id})">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-danger" 
                                onclick="deleteMouse(${mouse.id})">
                        ...
                        mouseTable.row.add([
                            ...
                            mouse.assigned_to || '',
                            mouse.emp_id || '',
                            formatDate(mouse.assign_date),
                            statusToggle,
                            formatDate(mouse.created_at),
                            actions
                        ]);
                    });
                    mouseTable.draw();
        ...
        function editMouse(id) {
            $.ajax({
                url: '<?= base_url('admin/mouse/product/') ?>' + id,
                method: 'GET',
                success: function(response) {
                    $('#mouseId').val(response.id);
                    $('#asset_id').val(response.asset_id);
                    $('#manufacturer').val(response.manufacturer);
                    $('#mouse_type').val(response.mouse_type);
        ...
        function saveMouse() {
            const id = $('#mouseId').val();
            const url = id ? '<?= base_url('admin/mouse/product/updateMouseProduct/') ?>' + id : '<?= base_url('admin/mouse/product/createMouseProduct') ?>';
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        loadMouses();
                        toastr.success('Mouse ' + (id ? 'updated' : 'created') + ' successfully');
                    } else {
        ...
        function deleteMouse(id) {
            if (confirm('Are you sure you want to delete this mouse?')) {
                $.ajax({
                    url: '<?= base_url('admin/mouse/product/deleteMouseProduct/') ?>' + id,
                    method: 'DELETE',
                    success: function(response) {
                        if (response.success) {
                            loadMouses();
                            toastr.success('Mouse deleted successfully');
                        } else {
                            toastr.error(response.message);
                        }
                    }
        ...
        function toggleStatus(id) {
            $.ajax({
                url: '<?= base_url('admin/mouse/product/updateMouseProduct/') ?>' + id,
                method: 'POST',
                data: {
        ...
        function assignMouse() {
            ...
            // Make AJAX call
            $.ajax({
                url: '<?= base_url('admin/mouse/product/assignMouse') ?>',
                method: 'POST',
                data: data,
                success: function(response) {
                    if (response.success) {
                        ...
                        loadMouses(); // Refresh the table
                    } else {
                        let errorMessage = response.message || 'Failed to assign mouse';
                        if (response.errors) {
                ...
                error: function(xhr, status, error) {
                    let errorMessage = 'Error occurred while assigning mouse';
                    try {
        ...
        function openUnassignModal() {
            ...
            // Fetch asset data
            $.ajax({
                url: '<?= base_url('admin/mouse/product/getAllAssignMouseProducts') ?>',
                method: 'GET',
                success: function(response) {
                    response.data.forEach(asset => {
                        assetSelect.append(
        ...
        // Function to handle unassign mouse
        function unassignMouse() {
            ...
            // Make AJAX call
            $.ajax({
                url: '<?= base_url('admin/mouse/product/unassignMouse') ?>',
                method: 'POST',
                data: data,
                success: function(response) {
                    if (response.success) {
